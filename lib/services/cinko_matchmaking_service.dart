...
import 'online_cinko_service.dart';

enum CinkoMmStatus { idle, searching, matched, cancelled, timeout, error }

class CinkoMmState {
  final CinkoMmStatus status;
  final String? matchId;
  final String? message;
  final String? opponentName;

  const CinkoMmState({
    this.status = CinkoMmStatus.idle,
    this.matchId,
    this.message,
    this.opponentName,
  });
...
class CinkoMatchmakingService {
  CinkoMatchmakingService._();
  ...
  static DatabaseReference get _queueRef => _db.ref('matchmaking/cinkoQueue');
  ...
  static Future<void> startSearch({
    required void Function(CinkoMmState) onUpdate,
    String? displayName,
  }) async {
    ...
    onUpdate(const CinkoMmState(
      status: CinkoMmStatus.searching,
      message: 'Çinko rakibi aranıyor…',
    ));
  ...
  static Future<void> _tryClaim(
    String uid,
    String myName,
    void Function(CinkoMmState) onUpdate,
  ) async {
    ...
    try {
      ...
      final matchId = await OnlineCinkoService.createRankedMatch(
        player1Uid: uid,
        player1Name: myName,
        player2Uid: bestUid,
      ...
      onUpdate(CinkoMmState(
        status: CinkoMmStatus.error,
        message: 'Hata: $e',
      ));
    } finally {
