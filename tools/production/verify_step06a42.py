...
try:
    ...
    for client in data.get("client") or []:
        ...
        if pkg == PACKAGE:
            matching.append(str(info.get("mobilesdk_app_id") or ""))

    if PACKAGE not in packages:
        errors.append(
            "google-services package mismatch: " + ", ".join(packages)
        )

    if matching:
        text = options.read_text(encoding="utf-8", errors="replace")
        if f"appId: '{matching[0]}'" not in text:
...
if errors:
    print("[FAIL] Step 06A.4.2 verification")
    for error in errors:
        print("  -", error)
    raise SystemExit(1)

print("[PASS] Firebase production package synchronized.")
print("[PASS] Firebase Android appId synchronized.")
print("[PASS] Android API36 + release signing intact.")
print("[OK] STEP 06A.4.2 PASS")
