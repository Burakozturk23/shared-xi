...
REPORT = ROOT / "reports/production/07b347"
...
def main():
    ...
    buckets = {
        "safe_commit_candidates.txt": safe,
        "optional_documentation.txt": docs,
        "local_tooling_do_not_commit.txt": tooling,
    ...
    print("=" * 70)
    print("LINKBALL STEP 07B.3.4.7 - SAFE COMMIT MANIFEST V2")
    print("=" * 70)
    print()
    print(f"Changed files        : {len(files)}")
    print(f"SAFE production      : {len(safe)}")
    print(f"Optional docs        : {len(docs)}")
    print(f"Local tooling        : {len(tooling)}")
    ...
    print("MANUAL REVIEW:")
    if review:
        for rel in review[:80]:
            print("  [REVIEW]", rel)
        if len(review) > 80:
            print(
                f"  [INFO] {len(review)-80} more saved to "
                "reports\\production\\07b347\\manual_review.txt"
            )
    else:
        print("  (none)")
    print()
    ...
    print("SUMMARY:")
    print("  SAFE production files are intended for the release commit.")
    print("  Optional docs can be committed later, separately.")
    print("  Local tooling/run_step*.bat should stay local.")
    print("  Generated/report/backup files should stay out of Git.")
    print()
    ...
    if review:
        ...
        print("[NEXT] Safe staging can be generated from the SAFE list.")
