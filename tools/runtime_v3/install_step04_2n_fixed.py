...
def main() -> None:
    ...
    if all(marker in text for marker in final_markers_for_idempotency):
        print("[PASS] odd_club_controller.dart already fully migrated.")
        return

    if any(marker in text for marker in final_markers_for_idempotency):
        raise RuntimeError(
            "PARTIAL_04_2N: controller kismi migration durumunda. "
            "04.2N.1 repair once temiz tabani geri yuklemeli."
        )
